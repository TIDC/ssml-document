    ...
    optionsExport(provider?: ServiceProvider) {
        ...
        switch (provider) {
            ...
            case ServiceProvider.Huoshanyun:
                options.rate = util.isFinite(Number(options.rate)) ? Number(((options.rate * 2 -1) * 10).toFixed(0)) : options.rate;
                options.pitch = util.isFinite(Number(options.pitch)) ? Number((Math.log(options.pitch) / Math.log(Math.E) * 12 / 0.69314718056 +10).toFixed(0)) : options.pitch;
